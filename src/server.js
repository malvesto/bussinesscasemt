...
server.use((req, res, next) => {
    if (req.method === 'POST') {
        ...
        const lastEmployee = employees.value().slice(-1)[0]; // Obtém o último empregado na coleção

        // Verifica se há empregados no banco de dados
        if (lastEmployee) {
            // Gera o próximo ID como o último ID + 1
            req.body.id = lastEmployee.id + 1;
        } else {
            // Se não houver empregados, define o ID como 1
            req.body.id = 1;
        }
    }
...
// Middleware para retornar os detalhes de um empregado específico
server.get('/employee/:empid', (req, res) => {
    const empid = parseInt(req.params.empid); // Converte o empid para número inteiro
    const db = router.db; // Obtém o objeto de banco de dados do router
    const employees = db.get('employee'); // Obtém a coleção 'employee' do banco de dados
    const empDetail = employees.find({ id: empid }).value(); // Busca o empregado pelo ID

    if (empDetail) {
        res.json(empDetail);
    } else {
        res.status(404).json({ error: 'Empregado não encontrado' });
    }
});
